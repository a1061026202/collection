      ...
      console.log(data.data.taskVo[i].taskName)
         await dk(c);//三餐打卡
          await $.wait(3000) 
                    ...
                    for (let taskInfo of data.data.taskVo) {
                        taskName = taskInfo.taskName.indexOf('答对') > -1 ? "答题" : taskInfo.taskName
                        finishStatus = taskInfo.finishStatus
                        if (finishStatus == 0) {
                            console.log(`去完成任务【${taskName}】`)
                            switch (taskName) {
                                case '记录一次早餐':
                                    await jlzc(0)
                                    break
                                ...
                                case '浏览商品':
                                    ...
                                    break
                                default:
                                    console.log(`未匹配任务【${taskName}】`)
                                    break
                            }
                        }

                    }
                    console.log(`去完成任务【浏览会场】`)
                    await llhc()
...
//记录早餐
async function jlzc(dietType, timeout = 0) {
    return new Promise((resolve) => {
        ...
        let url = {
            ...
            body: `{"inputDate" : "${nyr()}","inputType" : 0,"dietType" : ${dietType},"totalCalories" : 46,"images" : [],"userDietDetailReq" : [{"foodNumber" : "100","ingestionHeat" : 46,"foodId" : "93916","unitId" : 18}]}`
        }
...
async function drink(timeout = 0) {
    return new Promise((resolve) => {

        let url = {
            url: `https://jk-agw-m.simeitol.com/api/health/myjkUserdataUserDrinkRecord/insertUserDrinkRecord`,
            headers: {
                "Host": "jk-agw-h.simeitol.com",
                "Origin": "https://a-jk-h5-m.simeitol.com",
                "myjk-version": "1.5.1.0",
            ...
            body: `{"drinkUint":0,"drinkWater":200}`
        }
...
async function llhc(timeout = 0) {
    return new Promise((resolve) => {

        let url = {
            url: `https://jk-agw-m.simeitol.com/api/activity/myjkAppTask/appDoTask?taskCode=dtc_h5_pages`,
            headers: {
                "Host": "jk-agw-h.simeitol.com",
                "Origin": "https://a-jk-h5-m.simeitol.com",
                "myjk-version": "1.5.1.0",
...
async function llsp(timeout = 0) {
    return new Promise((resolve) => {

        let url = {
            url: `https://jk-agw-m.simeitol.com/api/activity/myjkAppTask/appDoTask?taskCode=shopping_goods_detail`,
            headers: {
                "Host": "jk-agw-h.simeitol.com",
                "Origin": "https://a-jk-h5-m.simeitol.com",
                "myjk-version": "1.5.1.0",
